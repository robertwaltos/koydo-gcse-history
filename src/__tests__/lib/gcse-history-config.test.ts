...
import { EXAM_CONFIG } from "@/lib/gcse-history/config";

describe("GCSE History Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM193");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("gcse-history");
  });
